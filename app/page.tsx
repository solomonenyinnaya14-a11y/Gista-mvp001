...
type Post={id:string;body:string|null;content_type:string;media_url:string|null;category:string;status:string;created_at:string;author_id:string;voice_duration_seconds:number|null;profiles:{display_name:string|null;username:string|null;avatar_url:string|null}|null;likes:number;responses:number;liked:boolean;saved:boolean};
...
export default function HomePage(){
 ...
 const loadPosts=useCallback(async(currentUser:any)=>{
  setLoading(true);
  let query=supabase.from("posts").select("id,body,content_type,media_url,category,status,created_at,author_id,voice_duration_seconds,profiles(display_name,username,avatar_url)");
  if(currentUser){
 ...
 <div className="feed">{loading?<p>Loading Gists…</p>:posts.length===0?<div className="empty-state"><h3>No Gists yet</h3><p>{tab==="Following"?"Follow people to see their Gists here.":"Be the first person to start a Gist."}</p></div>:posts.map(p=><article className="post" key={p.id}><div className="post-head"><div className="avatar">{p.profiles?.avatar_url?<img src={p.profiles.avatar_url} alt="" style={{width:"100%",height:"100%",objectFit:"cover",borderRadius:"50%"}}/>:(p.profiles?.display_name?.[0]?.toUpperCase()??"G")}</div><div className="identity"><strong>{p.profiles?.display_name??"Gista User"}</strong><span>@{p.profiles?.username??"user"} · {new Date(p.created_at).toLocaleString()}</span></div><span className="category">{p.category}</span></div>{p.content_type==="photo"&&p.media_url&&<img src={p.media_url} alt="Gist photo" style={{width:"100%",borderRadius:16,marginTop:10}}/>}{p.content_type==="voice"&&p.media_url&&<VoiceNote src={p.media_url} durationHint={p.voice_duration_seconds} />}{p.body&&<p className="post-text">{p.body}</p>}<div className="gist-status"><span className={p.status==="trending"?"hot":"dot"}>{p.status==="trending"?"🔥":"●"}</span>{p.status.charAt(0).toUpperCase()+p.status.slice(1)}<button onClick={()=>location.href="/gist/"+p.id}>Gist DNA</button></div><div className="actions"><button onClick={()=>toggleLike(p)} disabled={busy===p.id+"l"}><Heart size={18} fill={p.liked?"currentColor":"none"}/> {p.likes}</button><button onClick={()=>location.href="/gist/"+p.id}><MessageCircle size={18}/> {p.responses}</button><button onClick={()=>{const url=location.origin+"/gist/"+p.id;if(navigator.share)navigator.share({title:"Gista",text:p.body??"Join this Gist on Gista",url});else navigator.clipboard.writeText(url)}}><Share2 size={18}/></button><button onClick={()=>toggleSave(p)} disabled={busy===p.id+"s"}><Bookmark size={18} fill={p.saved?"currentColor":"none"}/></button></div></article>)}</div>
